...
private:
   ...
   void verifyArguments(int argc, char* argv[])
      {
      if (this->getVerboseMode()) { delete verboseMode; }
      if (this->getShortMessageMode()) { delete shortMessageMode; }
      ...
      if (!this->getVerboseMode()) { setVerboseMode(new MyBooleanClass()); }  // default is false
      if (!this->getShortMessageMode()) { setShortMessageMode(new MyBooleanClass()); }  // default is false
      }
